...
const furtherInstructions =
    "Please comment `@bot ready for review` when you've passed all checks, resolved merge conflicts and are ready to request a review.";
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        if (!(yield isPRMarkedReadyForReview())) return; // needed because synchronise event triggers this workflow on even draft PRs
        const prLabels = yield octokit.rest.issues
            .get({
                owner,
                repo,
                issue_number,
            ...
            .then(res => {
                core.info(res.headers["x-ratelimit-remaining"]);
                return res;
            })
...
function wasAuthorLinkedToFailingChecks() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const events = yield octokit.rest.issues
            .listEvents({
                owner,
                repo,
                issue_number,
        ...
        const labelEvent = events.find(e => {
            var _a;
            return (
                e.event === "labeled" && ((_a = e.label) === null || _a === void 0 ? void 0 : _a.name) == "s.Ongoing"
            );
        });
        if (!labelEvent) {
            core.warning(
                "Some wrong assumption may have been made or the API used to fetch the PRs may have changed. This function should have been called only on PRs that are assigned the label."
            );
            return true; // skip adding a comment
        }
        // // get an array of events for the current issue (https://octokit.github.io/rest.js/v18#issues-list-events)
        const comments = yield octokit.rest.issues
            .listComments({
                owner,
                repo,
                issue_number,
...
function isPRMarkedReadyForReview() {
    return __awaiter(this, void 0, void 0, function* () {
        return yield octokit.rest.pulls
            ...
            .catch(err => {
                common_1.log.info(err, "error getting pr that triggered this workflow");
                throw err;
            });
    });
}
